```{r context="server"}
if (!exists("etude2_app_registration")) 
  etude2_app_registration <- reactiveValues()


shiny_state <- shiny::eventReactive(input$get_state, {
  # get the state from etude2_app_registration
  res <- list()
  nms <- names(etude2_app_registration)
  if (length(nms) == 0) return(NULL)
  for (k in 1:length(nms)) {
    res[[k]] <- etude2_app_registration[[nms[k]]]
  }
